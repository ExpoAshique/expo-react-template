...
class Table extends TableContext {
  ...
  componentDidMount() {
    ...
    let table = window.$('#datatable-buttons').DataTable({
      pageLength: this.props.pageSize || 10,
      // buttons: this.props.nuButton ? [] : ['copy', 'excel', 'pdf', 'colvis'],
      scrollX: true,
      paging: false,
      bInfo: false,
      searching: false,
  ...
  render() {
    ...
    return (
      ...
      this.props.heading ?
        <div>
          <table id="datatable-buttons" className={"table table-striped" + (!wrap ? " nowrap" : "")} style={{ borderCollapse: 'collapse', borderSpacing: 0, width: '100%',marginTop: -7 }}>
